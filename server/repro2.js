// Faithful browser simulation: the guest's loadRom is SYNCHRONOUS and heavy
// (like the real jsnes loadROM), which blocks the guest's event loop for a
// while. This mirrors netplay.js exactly and runs past STALL_TIMEOUT to see
// if the guest's stall guard fires (guest disconnects -> host gets peer-left).
'use strict';
...
function makeClient(name, isHost) {
  ...
  c.handlePeer = (data) => {
    ...
    if (data.type === 'guest-joined' && c.role === 'host') {
      ...
      const bytes = 'F'.repeat(65536); // bigger fake ROM
      c.send({ type: 'p2p', data: { type: 'rom', name: 'g.nes', bytes } });
      return;
    }
    if (data.type === 'rom' && c.role === 'guest') {
      c.log(`got ROM (${data.bytes.length} bytes) -> loading SYNCHRONOUSLY (blocks event loop)`);
      // Simulate the heavy jsnes loadROM: busy-wait ~300ms synchronously.
      const end = Date.now() + 300;
      while (Date.now() < end) { /* busy */ }
      c.log('loadROM done -> sending ready (still not romReady)');
...
const timer = setInterval(() => {
  ...
  guest.step();
  frames++;
  if (frames > 400) { // 8 seconds > STALL_TIMEOUT
    clearInterval(timer);
    console.log('\n=== RESULT ===');
